...
class changePasswordDialog(QDialog):
    ...
    def setUpUI(self):
        ...
        self.studentIdLabel = QLabel("用 户 名：")
        ...
        self.titlelabel.setStyleSheet("font: 18pt; font: bold")
        font.setPixelSize(16)
        self.studentIdLabel.setStyleSheet("font: 14pt")

        self.oldPasswordLabel.setStyleSheet("font: 14pt")
        self.passwordLabel.setStyleSheet("font: 14pt")
        self.confirmPasswordLabel.setStyleSheet("font: 14pt")

        font.setPixelSize(16)
        self.studentIdEdit.setStyleSheet("font: 14pt")
        self.changePasswordButton.setStyleSheet("font: 14pt")
    ...
    def changePasswordButtonClicked(self):
        ...
        if studentId == "" or oldPassword == "" or password == "" or confirmPassword == "":
            QMessageBox.warning(self, "警告", "输入不可为空，请重新输入", QMessageBox.Yes, QMessageBox.Yes)
            return
        # 密码与确认密码不同
        if password != confirmPassword:
            QMessageBox.warning(self, "警告", "两次输入密码不同,请重新输入", QMessageBox.Yes, QMessageBox.Yes)
